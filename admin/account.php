...
if (isset($_POST['submit'])) {
	$useremail = mysqli_real_escape_string($conn, $usermail);
    $email = $_POST['mail'];
    $encpass = md5($_POST['pass']);
	$update_pass = "UPDATE users SET email = $email, password = '$encpass' WHERE email = '$useremail'";
    $run_query = mysqli_query($conn, $update_pass);
        if($run_query){
            echo "Account Updated.\n";
			$getlogfile = file_get_contents("log.txt");
			$fp = fopen('log.txt', 'w');
			fwrite($fp, "User: ".$_SESSION["email"]." Action: Account Update. | IP Address: $ip | User Agent: ".$_SERVER['HTTP_USER_AGENT']." Date: ".date("M/d/Y H:i:s")."\n".$getlogfile);
			fclose($fp);
			session_destroy();
			header("Location: index.php");
        } else {
            echo "ERROR: Could not able to execute $run_query. " . mysqli_error($conn);
        }
        mysqli_close($conn);
};
?>
...
<button id="tr" class="translate btn">Turkish</button>
    ...
    <form action="" method="POST">
    				...
    				<div class="col-xl-4">
    	            	<div class="row">
    						<div class="col-md-6 col-xl-12">
    							<div class="mb-3">
    	                		<label class="lang form-label" key="newmail">New Email</label>
    	                		<input type="text" class="form-control" placeholder="<?php echo("$usermail"); ?>" name="mail" required>
    	            		</div>
    						<div class="col-md-6 col-xl-12">
    							<div class="mb-3">
    	                		<label class="lang form-label" key="newpswd">New Password</label>
		...
		<div class="input-group">
			<a class="lang btn" key="addnewadminacc" href="new-account.php">Add New Admin Account</a>
		</div>
      </div>
    </div>
    <!-- Libs JS -->
